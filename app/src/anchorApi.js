...
import idl from './idl/anc1.json'
import { WalletAdaptorPhantom } from "./phantonWallet";
...
export default class AnchorApi {
	...
	constructor({ programId, config, keypair } = {}) {
		...
		const wallet = window.solana.isConnected && window.solana?.isPhantom
	...
	async initialize(bio) {
		...
		await this.program.rpc.initialize(utf8encoded, {
			// Pass accounts needed
			accounts: {
		...
		console.log(
			`intialized Blog ID: ${blogAccount.publicKey} for acct ${this.provider.wallet.publicKey}`
		);
		return blogAccount;
	}
